  ...
  return (
    <div className="space-y-5">
      <div className="bg-gmp-accent2 border border-gray-700 rounded-gmp p-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-gmp-text-secondary text-xs font-medium">N° de orden:</span>
            <span className="text-white text-sm font-bold">#{data.numeroorden}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gmp-text-secondary text-xs font-medium">Estado:</span>
            <span className="text-gmp-primary text-sm font-bold">{data.status}</span>
          </div>
          {data.fechaIngreso && (
            <div className="flex justify-between">
              <span className="text-gmp-text-secondary text-xs font-medium">Fecha ingreso:</span>
              <span className="text-white text-sm font-bold">{formatDate(data.fechaIngreso)}</span>
            </div>
          )}
          {data.fechaSalida && (
            <div className="flex justify-between">
              <span className="text-gmp-text-secondary text-xs font-medium">Fecha finalización:</span>
      ...
      <div className="bg-gmp-accent2 border border-gray-700 rounded-gmp p-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-gmp-text-secondary text-xs font-medium">Placa:</span>
            <span className="text-white text-sm font-bold uppercase">{data.placa}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gmp-text-secondary text-xs font-medium">Marca:</span>
            <span className="text-white text-sm font-bold">{data.marca}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gmp-text-secondary text-xs font-medium">Modelo:</span>
      ...
      <div className="space-y-4">
        ...
        {data.diagnosticos.map((diag, idx) => (
          <div key={idx} className="bg-gmp-accent2 border border-gray-700 rounded-gmp p-4">
            ...
            <p className="text-gmp-text-secondary text-sm mb-4">{diag.solucion}</p>

            {diag.fotos && diag.fotos.length > 0 && (
              <div>
                <p className="text-gmp-text-secondary text-xs font-medium mb-2">Imágenes de finalización</p>
